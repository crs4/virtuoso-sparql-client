...
module.exports = class Client {
  ...
  store(echo = false){
    ...
    let emptySize = this._calculateEmptyQuerySize();
    let triplesString = this.getLocalStore().toTriplePattern();
    let totalSize = Buffer.byteLength(emptySize + " " +triplesString, 'utf8');
    ...
    for (let triple of this.getLocalStore()) {
      if(Buffer.byteLength(emptySize + "  " + triple.toTriplePattern(), 'utf8') >= 9900){
        throw new Error(`A single triple exceeds the maximum size for a query
          [Subject: ${triple.getSubject()} - Property: ${triple.getPredicate()}]`);
      }

      if(Buffer.byteLength(emptySize + "  " + patterns + "  " + triple.toTriplePattern(), 'utf8') < maximum){
        patterns += `
    ...
    //console.log(`SIZE: ${totalSize} - QUERIES: ${queries.length}`);
    ...
    queries.forEach((currentQuery)=>{
      ...
      promises.push(this.query(currentQuery, echo));
    });
    return Promise.all(promises);
  }
  storeVersion(echo = false){
    this.methodName = "storeVersion";
    if(!this.versioning){
      throw new Error(`Versioning not enabled, please use Client.setVersioning() or Client.setOptions() prior to use versioning`);
    }
    if(!Boolean(this._calculateVersioningQueryGraph()))
      throw new Error(`Graph name is empty.\nUse the methods 'setDefaultGraph()' or 'setQueryGraph()' to set it`);
    ...
    for (let triple of this.getLocalStore()) {
      ...
      if(Buffer.byteLength(emptySize + "  " + patterns + "  " + triple.toTriplePattern() + "  " + triple.getAnnotationPattern(), 'utf8') < maximum){
        patterns += `
          ${triple.toTriplePattern()}
          ${triple.getAnnotationPattern()}`;
      }
      else{
        queryString = `INSERT IN GRAPH <${this._calculateVersioningQueryGraph()}> {
          ${patterns}
        }`;
        queries.push(queryString);
        patterns += `
  ...
  storeRemove(echo = false){
    this.methodName = "versioning";
    if(!this.versioning){
      throw new Error(`Versioning not enabled, please use Client.setVersioning() or Client.setOptions() prior to use versioning`);
    }
    if(!Boolean(this._calculateQueryGraph()))
    ...
    for (let triple of this.getLocalStore()) {
      ...
      if(Buffer.byteLength(emptySize + "  " + patterns + "  " + triple.toTriplePattern(), 'utf8') < maximum){
        ...
        queryString = `WITH <${this._calculateQueryGraph()}>
          DELETE { ${patterns} } 
          WHERE { ${patterns} }`;
        queries.push(queryString);
        patterns += `
  ...
  updateVersion(echo = false){
    this.methodName = "updateVersion";
    if(this.getLocalStore().triples.length === 0){
      return Promise.resolve();
    }

    let storeDivided = this._splitStoreByAuthoringType();
    let storeDuplicated = this.getLocalStore().triples;

    return new Promise((resolve, reject) => {
      this.getLocalStore().empty();
      storeDivided.removes.forEach((i) => this.getLocalStore().add(i));
      
      return this.storeRemove(echo)
        .then(() => {
          this.getLocalStore().empty();
          storeDivided.adds.forEach((i) => this.getLocalStore().add(i));
          return this.store(echo);        
        })
        .then(() => {
          this.getLocalStore().empty();
          storeDuplicated.forEach((i) => this.getLocalStore().add(i));

          return this.storeVersion(echo);
        })
        .then(() => {
          return resolve();
        })
  ...
  setOptions(format = "application/ld+json", prefixes = {}, graph = "", versioning = false){
  ...
  _calculateEmptyQuerySize(){
    let emptyQuery = `INSERT DATA {
      GRAPH <${this._calculateQueryGraph()}> {
    ...
    emptyQuery = `
      ...
      ${emptyQuery}
    `;
    return Buffer.byteLength(emptyQuery, 'utf8')
  }
  ...
  _calculateVersioningQueryGraph(){
    let value = (this.queryGraph) ? this.queryGraph+'-versions' : (this.defaultGraph) ? this.defaultGraph+'-versions' : "";
    return value;
  }
  _resetQueryData(){
  ...
  _splitStoreByAuthoringType(){
    let adds = [];
    let removes = [];
    for (let triple of this.getLocalStore()) {
      if(triple.hasAuthoring()){
        if(triple.getAuthoring().type.value === Authoring.isAdd()){
          adds.push(triple);
        }
        if(triple.getAuthoring().type.value === Authoring.isRemove()){
          removes.push(triple);
        }
      }
    }
