  ...
  store(echo = false){
    ...
    let queryString = `INSERT DATA {
      GRAPH <${this._calculateQueryGraph()}> {
        ${this.localStore.toTriplePattern()}
      }
    }`;
    this.localStore.clean();

    this.setQueryPrefixes(this.localStore.getPrefixes());
    this.query(queryString, echo);
  }
  query(query, echo = false){
    return new Promise((resolve, reject) => {
      let opts = this._createQueryOptions(query, echo);
      let methodName = (this.methodName) ? this.methodName : "query";
